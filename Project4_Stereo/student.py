...
def project_impl(K, Rt, points):
    ...
    projections = projections_matrix[:2, :].T.reshape(h, w, 2)
    return projections
    # raise NotImplementedError()
...
def preprocess_ncc_impl(image, ncc_size):
    ...
    for i in range(h):
        for j in range(w):
            if i < ncc_half or i > h - ncc_half - 1 or j < ncc_half or j > w - ncc_half - 1:
                patch_R.append(np.zeros(ncc_size ** 2))
                patch_G.append(np.zeros(ncc_size ** 2))
                patch_B.append(np.zeros(ncc_size ** 2))
            else:
...
def compute_ncc_impl(image1, image2):
    ...
    image1_flat = image1.reshape(h * w, -1)
    image2_flat = image2.reshape(h * w, -1)
    ncc_flat = np.multiply(image1_flat, image2_flat)
    ncc_tmp = np.sum(ncc_flat, axis=1)
    ncc = ncc_tmp.reshape(h, w)
